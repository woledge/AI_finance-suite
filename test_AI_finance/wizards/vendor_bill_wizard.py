...
class VendorBillWizard(models.TransientModel):
    _name = 'test.ai.vendor.bill.wizard'
    ...
    line_ids = fields.One2many(
        'test.ai.vendor.bill.wizard.line',
        'wizard_id',
    ...
    created_refund_id = fields.Many2one('test.ai.refund.tracking', string='Created Refund')
    ...
    column_ids = fields.One2many(
        'test.ai.vendor.bill.wizard.column', 'wizard_id',
        string='Extracted Columns'
    )
    selected_column_id = fields.Many2one(
        'test.ai.vendor.bill.wizard.column',
...
"""
    ...
    def _populate_from_statement_data(self, data: dict):
        ...
        for item in data.get('items', []):
            self.env['test.ai.vendor.bill.wizard.line'].create({
                'wizard_id': self.id,
    ...
    def _create_statement_direct(self):
        ...
        for col_name in column_names:
            ...
            rec = self.env['test.ai.vendor.bill.wizard.column'].create({
                'wizard_id': self.id,
    ...
    def _create_and_match_statement(self, data=None):
        ...
        statement = self.env['test.ai.vendor.statement'].create(stmt_vals)

        for item in items:
            self.env['test.ai.vendor.statement.line'].create({
        ...
        for col_name in column_names:
            is_rec = (col_name.strip().lower() == recommended_col.strip().lower()) if recommended_col else False
            col_rec = self.env['test.ai.vendor.statement.column'].create({
                'statement_id': statement.id,
                'column_name': col_name.strip(),
                'is_recommended': is_rec,
            })
        ...
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'test.ai.vendor.statement',
            'res_id': statement.id,
            'view_mode': 'form',
            'target': 'current',
        }
...
"""
    ...
    def action_create_bank_statement(self):
        ...
        statement = self.env['test.ai.bank.statement'].create(stmt_vals)
        ...
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'test.ai.bank.statement',
            'res_id': statement.id,
            'view_mode': 'form',
            'target': 'current',
        }
    ...
    def action_create_statement_from_review(self):
        ...
        if existing_stmt_id:
            extracted_cols_str = data.get('_extracted_columns', self.extracted_columns_display or '')
            statement = self.env['test.ai.vendor.statement'].browse(existing_stmt_id)
            if statement.exists():
                update_vals = {
...
"""
    ...
    def _populate_from_refund_data(self, data: dict):
        ...
        if line_items:
            ...
            if vals['subtotal'] > 0:
                self.env['ai.vendor.bill.wizard.line'].create({
                    'wizard_id': self.id,
    ...
    def action_create_refund(self):
        ...
        existing = self.env['test.ai.refund.tracking'].search([
        ...
        refund = self.env['test.ai.refund.tracking'].create(refund_vals)
...
class VendorBillWizardLine(models.TransientModel):
    _name = 'test.ai.vendor.bill.wizard.line'
    _description = 'Vendor Bill Wizard Line'
    
    wizard_id = fields.Many2one(
        'test.ai.vendor.bill.wizard',
        string='Wizard',
        required=True,
        ondelete='cascade'
    )
...
class VendorBillWizardColumn(models.TransientModel):
    _name = 'test.ai.vendor.bill.wizard.column'
