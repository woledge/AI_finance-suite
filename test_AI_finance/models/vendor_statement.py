...
class VendorStatement(models.Model):
    _name = 'test.ai.vendor.statement'
    ...
    line_ids = fields.One2many('test.ai.vendor.statement.line', 'statement_id', string='Statement Lines')
    ...
    statement_column_ids = fields.One2many(
        'test.ai.vendor.statement.column', 'statement_id',
        string='Extracted Columns'
    )
    selected_column_id = fields.Many2one(
        'test.ai.vendor.statement.column',
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('test.ai.vendor.statement') or 'New'
    ...
    def action_match_lines(self):
        ...
        for voucher_key, line_ids in matched_vouchers.items():
            if len(line_ids) > 1:
                for lid in line_ids:
                    dup_line = self.env['test.ai.vendor.statement.line'].browse(lid)
                    if dup_line.exists():
                        dup_line.write({
    ...
    def action_print_report(self):
        ...
        return self.env.ref('test_AI_finance.action_report_vendor_statement').report_action(self)


class VendorStatementLine(models.Model):
    _name = 'test.ai.vendor.statement.line'
    _description = 'Partner Statement Line Item'
    _order = 'date, id'

    statement_id = fields.Many2one('test.ai.vendor.statement', string='Statement', ondelete='cascade')
    ...
    candidate_move_line_ids = fields.Many2many('account.move.line',
                                               'vendor_stmt_line_candidate_rel',
...
class VendorStatementColumn(models.Model):
    _name = 'test.ai.vendor.statement.column'
    _description = 'Extracted Column for Statement Matching'
    _rec_name = 'display_label'

    statement_id = fields.Many2one(
        'test.ai.vendor.statement',
        string='Statement',
        required=True,
        ondelete='cascade'
    )
