...
export class VirtualCFOChat extends Component {
    static template = "ai_finance_suite.VirtualCFOChat";
    static props = ["*"];
    ...
    async createNewSession() {
        try {
            const newId = await this.orm.create("af.virtual.cfo.agent", [{}]);
    ...
    async deleteSession(sessionId, event) {
        ...
        try {
            ...
            // Delete the session
            await this.orm.unlink("af.virtual.cfo.agent", [sessionId]);
...
registry.category("actions").add("af_virtual_cfo_chat", VirtualCFOChat);
