...
class AIFinanceDashboard(models.Model):
    ...
    def _compute_module_stats(self):
        for rec in self:
            ...
            try:
                rec.total_statements_reconciled = self.env['af.vendor.statement'].search_count([
            ...
            # Bank statements
            try:
                rec.total_bank_statements = self.env['af.bank.statement'].search_count([])
            except Exception:
                rec.total_bank_statements = 0

            # Refunds tracked
            try:
                rec.total_refunds_tracked = self.env['af.refund.tracking'].search_count([])
            except Exception:
    ...
    def action_refresh(self):
        ...
        return {
            ...
            'res_model': 'af.finance.dashboard',
            'res_id': self.id,
            'view_mode': 'form',
            'target': 'current',
        }
