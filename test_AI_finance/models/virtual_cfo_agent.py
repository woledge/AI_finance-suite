...
class VirtualCFOAgent(models.Model):
    _name = 'test.ai.virtual.cfo.agent'
    ...
    conversation_ids = fields.One2many('test.ai.cfo.conversation', 'cfo_agent_id',
    ...
    def ask_question(self, question):
        ...
        conversation = self.env['test.ai.cfo.conversation'].create({
        ...
        try:
            ...
            ai_model_name = ''
            if cfo_cred_id:
                cfo_cred = self.env['test.ai.credential'].sudo().browse(cfo_cred_id)
                if cfo_cred.exists():
    ...
    def _get_conversation_history(self, limit=6):
        ...
        conversations = self.env['test.ai.cfo.conversation'].search(
    ...
    def _get_reconciliation_summary(self):
        ...
        try:
            bank_statements = self.env['test.ai.bank.statement'].search([
                ('company_id', '=', company_id),
...
Provide a clear, helpful response following the formatting rules."""
    ...
    def action_new_conversation(self):
        ...
        return {
            ...
            'res_model': 'test.ai.cfo.query.wizard',
            'view_mode': 'form',
            'target': 'new',
...
class CFOConversation(models.Model):
    _name = 'test.ai.cfo.conversation'
    _description = 'Virtual CFO Conversation'
    _order = 'create_date desc'

    cfo_agent_id = fields.Many2one('test.ai.virtual.cfo.agent', string='CFO Session',
                                    ondelete='cascade')
