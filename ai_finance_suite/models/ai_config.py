...
class AIFinanceConfig(models.TransientModel):  # Inherit TransientModel for res.config.settings
    ...
    ocr_credential_id = fields.Many2one('af.credential', string='OCR AI Credential',
                                      config_parameter='ai_finance_suite.ocr_credential_id',
                                      help='Credential to use for OCR/VLM processing')
                                      
    virtual_cfo_credential_id = fields.Many2one('af.credential', string='Virtual CFO AI Credential',
                                              config_parameter='ai_finance_suite.virtual_cfo_credential_id',
    ...
    def get_config(self):
        ...
        # Get OCR Credential
        ocr_cred_id = int(ICP.get_param('ai_finance_suite.ocr_credential_id', 0))
        if ocr_cred_id:
            ocr_cred = self.env['af.credential'].browse(ocr_cred_id)
            if ocr_cred.exists():
        ...
        if cfo_cred_id:
            cfo_cred = self.env['af.credential'].browse(cfo_cred_id)
            if cfo_cred.exists():
